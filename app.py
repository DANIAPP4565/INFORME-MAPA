...
def _procesar_pdf_mapa_lote(
    ...
    noc_start = str(meta.get("noc_start") or noc_start_respaldo or DEFAULT_NIGHT_START)
    noc_end = str(meta.get("noc_end") or noc_end_respaldo or DEFAULT_NIGHT_END)

    if _clock_spec_to_minutes(noc_start) is None or _clock_spec_to_minutes(noc_end) is None:
        raise ValueError(f"Intervalo nocturno inválido: {noc_start}–{noc_end}.")
...
def render_procesamiento_lote_mapa(logo_file, firma_file, sello_file):
    ...
    intervalos_validos = (
        _clock_spec_to_minutes(noc_start_respaldo) is not None
        and _clock_spec_to_minutes(noc_end_respaldo) is not None
    )
    if not intervalos_validos:
        st.error("El intervalo nocturno de respaldo debe expresarse como HH:MM.")
