...
def _parse_medicaldb_table_text(text):
    ...
    for chunk in chunks:
        ...
        for mm in row_regex.finditer(chunk2):
            ...
            fecha = _normalize_date_token(mm.group("fecha"))
    ...
    if len(rows) < 20:
        ...
        for line in text.splitlines():
            ...
            for tok in tokens:
                if fecha is None:
                    fecha = _normalize_date_token(tok)
                    if fecha: 
                        continue
...
def _extract_meta(text):
    """Extrae filiatorios automáticamente desde PDF/OCR del equipo."""
    meta = {}
    if not text:
        return meta
    t = re.sub(r"[ \t]+", " ", text)
    ...
    # Documento
    m = re.search(r"Documento[:\s]+(\d{6,10})", t, re.I)
    if m:
        meta["documento"] = m.group(1)

    # Fecha de nacimiento y edad calculada
    m = re.search(r"Fecha\s+de\s+Nacimiento[:\s]+(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4})", t, re.I)
    if m:
        fnac = _normalize_date_token(m.group(1))
        meta["fecha_nacimiento"] = fnac

    # Sexo
    m = re.search(r"Sexo[:\s]+([MF])", t, re.I)
    if m:
        meta["sexo"] = "Femenino" if m.group(1).upper() == "F" else "Masculino"

    # Obra social
    m = re.search(r"Obra\s+Social[:\s]+([A-ZÁÉÍÓÚÑ0-9 ._-]+?)(?:Número|Numero|Datos del estudio|\n)", t, re.I)
    if m:
    ...
    # Inicio / Fin
    m = re.search(r"Inicio[:\s]+(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4})\s+(\d{1,2}:\d{2}(?::\d{2})?)\s+Fin[:\s]+(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4})\s+(\d{1,2}:\d{2}(?::\d{2})?)", t, re.I)
    if m:
        meta["fecha_inicio"] = _normalize_date_token(m.group(1))
        meta["hora_inicio"] = _normalize_time_token(m.group(2))
        meta["fecha_fin"] = _normalize_date_token(m.group(3))
        meta["hora_fin"] = _normalize_time_token(m.group(4))
    else:
        dates = re.findall(r"\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}", t)
        if dates:
            meta["fecha_inicio"] = _normalize_date_token(dates[0])

    # Edad desde fecha nacimiento y fecha estudio
    try:
