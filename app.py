...
# ─────────────────────────────────────────────────────────────────────────────
# CONFIGURACIÓN OCR DEL SISTEMA
# ─────────────────────────────────────────────────────────────────────────────
def configurar_tesseract():
    """
    Detecta y valida el ejecutable Tesseract OCR.

    Funciona en:
    - Streamlit Cloud / Linux: busca ``tesseract`` en PATH.
    - Windows: revisa ubicaciones habituales de instalación.
    - macOS/Homebrew: revisa rutas Intel y Apple Silicon.
    - Entornos personalizados: respeta la variable TESSERACT_CMD.

    Returns
    -------
    tuple[bool, str | None, str]
        (disponible, ruta_ejecutable, detalle)
    """
    try:
        import pytesseract
    except Exception as e:
        return False, None, f"No se pudo importar pytesseract: {e}"

    candidates = []

    # 1) Ruta explícita configurada por el usuario/servidor.
    env_cmd = os.environ.get("TESSERACT_CMD", "").strip()
    if env_cmd:
        candidates.append(env_cmd)

    # 2) Ejecutable visible en PATH. Es el caso normal en Streamlit Cloud
    # cuando packages.txt contiene tesseract-ocr.
    ...
    for cmd in unique_candidates:
        # Para rutas absolutas exigir existencia; para nombre simple, resolver PATH.
        resolved = cmd
        ...
        try:
            ...
            # Verificación real: no basta con importar pytesseract.
            proc = subprocess.run(
                [resolved, "--version"],
            ...
            if proc.returncode == 0 and output:
                ...
                return True, resolved, first_line

            errors.append(
                f"{resolved}: código de salida {proc.returncode}; "
                f"{(proc.stderr or proc.stdout or '').strip()[:180]}"
            )
        except Exception as e:
            errors.append(f"{resolved}: {e}")

    # Último intento: dejar que pytesseract use su comando por defecto.
    try:
        version = pytesseract.get_tesseract_version()
        cmd = getattr(pytesseract.pytesseract, "tesseract_cmd", "tesseract")
        return True, str(cmd), f"tesseract {version}"
    except Exception as e:
        errors.append(f"Comando por defecto: {e}")

    detail = (
        "Tesseract OCR no está instalado o no es ejecutable. "
        f"Sistema: {platform.system()} {platform.release()}."
    )
    if errors:
        detail += " Intentos: " + " | ".join(errors[-4:])
    return False, None, detail


def requerir_tesseract():
    """
    Garantiza que el motor OCR esté operativo antes de ejecutar OCR.
    Lanza RuntimeError con instrucciones concretas si falta.
    """
    ok, path, detail = configurar_tesseract()
    if not ok:
        raise RuntimeError(
            "Motor Tesseract OCR no disponible. "
            "Local: instalar Tesseract OCR en el sistema y luego "
            "`pip install PyMuPDF Pillow pytesseract opencv-python-headless`. "
            "Streamlit Cloud: agregar `tesseract-ocr`, `tesseract-ocr-spa` y "
            "`tesseract-ocr-eng` en packages.txt. "
...
def estado_ocr():
    """
    Comprueba bibliotecas Python y motor Tesseract.
    No ejecuta OCR sobre documentos; sólo valida disponibilidad.
    """
    try:
        import fitz  # PyMuPDF
        ...
        try:
            ...
            return {
                ...
                "version": None,
                "languages": [],
                "error": f"Falta OpenCV (opencv-python-headless): {e}",
            }
    except Exception as e:
        return {
            "ok": False,
            ...
            "version": None,
            "languages": [],
            "error": f"Faltan dependencias Python OCR: {e}",
        }
...
def _validate_night_window(noc_start, noc_end, *, strict_cross_midnight=True):
    """
    Valida que el intervalo nocturno represente realmente una ventana de sueño
    nocturna y no un intervalo diurno o el inicio/fin del estudio extraído por OCR.

    Regla de seguridad MAPA:
    - inicio nocturno entre 18:00 y 23:59;
    - fin nocturno entre 00:00 y 11:59;
    - duración entre 4 y 14 horas;
    - por defecto debe cruzar medianoche.

    Devuelve un diccionario con ``ok``, horas normalizadas y motivo.
    """
    ...
    if start_min is None or end_min is None:
        return {
            ...
            "start": None,
            "end": None,
            "reason": "Formato horario inválido; usar HH:MM.",
        }
    if start_min == end_min:
        return {
            ...
            "reason": "El inicio y el fin nocturnos no pueden ser iguales.",
        }

    crosses_midnight = start_min > end_min
    duration = (end_min - start_min) % 1440
    plausible_clock = (18 * 60 <= start_min <= 23 * 60 + 59) and (0 <= end_min < 12 * 60)
    plausible_duration = 4 * 60 <= duration <= 14 * 60

    if strict_cross_midnight and not crosses_midnight:
        return {
            "ok": False,
            "start": _minutes_to_hhmm(start_min),
            "end": _minutes_to_hhmm(end_min),
            "reason": (
                "El período nocturno no cruza medianoche. Esto suele indicar que el OCR "
                "confundió el horario diurno o el inicio del estudio con la noche."
            ),
        }
    if not plausible_clock or not plausible_duration:
        return {
            "ok": False,
            "start": _minutes_to_hhmm(start_min),
            "end": _minutes_to_hhmm(end_min),
            "reason": (
                "Ventana nocturna clínicamente improbable para MAPA "
                f"({_minutes_to_hhmm(start_min)}–{_minutes_to_hhmm(end_min)}, "
                f"duración {duration/60:.1f} h)."
            ),
        }
    ...
    return {
        ...
        "reason": "Intervalo nocturno válido.",
    }


def _safe_night_window(noc_start, noc_end):
    """Devuelve una ventana nocturna segura; si falla, usa 23:00–07:00."""
    qc = _validate_night_window(noc_start, noc_end, strict_cross_midnight=True)
    if qc["ok"]:
        return qc["start"], qc["end"], None
    return DEFAULT_NIGHT_START, DEFAULT_NIGHT_END, qc["reason"]
...
def _extract_meta(text):
    ...
    # Seguridad crítica: el OCR puede unir "inicio del estudio" con "23:00"
    # y producir una falsa noche 13:38–23:00. Sólo se acepta una ventana
    # nocturna plausible que cruce medianoche; en caso contrario se descarta
    # y se conserva el estándar 23:00–07:00.
    night_window = _extract_night_interval(flat_space)
    if night_window:
        qc_night = _validate_night_window(*night_window, strict_cross_midnight=True)
        if qc_night["ok"]:
            meta["noc_start"], meta["noc_end"] = qc_night["start"], qc_night["end"]
            meta["periodo_nocturno_fuente"] = "PDF del equipo (validado)"
        else:
            meta["noc_start"], meta["noc_end"] = DEFAULT_NIGHT_START, DEFAULT_NIGHT_END
            meta["periodo_nocturno_descartado"] = (
                f"{night_window[0]}–{night_window[1]}: {qc_night['reason']}"
            )
...
def assign_periods(
    ...
    qc_night = _validate_night_window(noc_start, noc_end, strict_cross_midnight=True)
    if not qc_night["ok"]:
        raise ValueError(
            "Intervalo nocturno inseguro: " + qc_night["reason"] +
            " Use una ventana que cruce medianoche, por ejemplo 23:00–07:00."
        )
    start_min = _clock_spec_to_minutes(qc_night["start"])
    end_min = _clock_spec_to_minutes(qc_night["end"])
...
def _try_import_ocr_stack():
    """
    Carga dependencias OCR y valida el motor Tesseract del sistema.
    Devuelve el mismo contrato histórico para mantener compatibilidad.
    """
    try:
        import fitz  # PyMuPDF
        from PIL import Image
        import pytesseract
        path, detail = requerir_tesseract()
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("Estado OCR", expanded=False):
            ocr_status = estado_ocr()
            if not ocr_status["ok"]:
                ...
                st.markdown(
                    "**Motor del sistema requerido:** `tesseract-ocr` "
                    "(y paquetes de idioma `spa`/`eng` cuando estén disponibles)."
                )
                st.caption(f"Detalle: {ocr_status['error']}")
            else:
                st.success("OCR disponible y motor Tesseract validado")
                st.caption(f"Ejecutable: {ocr_status['path']}")
                st.caption(f"Versión: {ocr_status['version']}")
                if ocr_status["languages"]:
                    st.caption(
                        "Idiomas OCR detectados: "
                        + ", ".join(ocr_status["languages"])
                    )
                    if "spa" not in ocr_status["languages"]:
                        st.warning(
                            "El idioma español (spa) no está instalado. "
                            "La app podrá usar inglés como respaldo, pero conviene "
                            "agregar tesseract-ocr-spa."
                        )
    ...
    if st.session_state.get("parse_error"):
        ...
        if is_gen:
            ...
            with st.expander("Diagnóstico técnico"):
                ...
                st.markdown("""
                Para PDFs escaneados se requiere OCR completo: bibliotecas Python y motor Tesseract.  
                **Local (Python):** `pip install PyMuPDF Pillow pytesseract opencv-python-headless`  
                **Local (sistema):** instalar Tesseract OCR y, de ser posible, los idiomas español e inglés.  
                **Streamlit Cloud:** subir `requirements.txt` y `packages.txt` en la raíz del repositorio y reiniciar la app.
                """)
        return
    ...
    noc_start_default, noc_end_default, night_default_warning = _safe_night_window(
        meta.get("noc_start", DEFAULT_NIGHT_START),
        meta.get("noc_end", DEFAULT_NIGHT_END),
    )
    ...
    st.caption(
        ...
        + (" (detectado y validado en PDF)" if meta.get("periodo_nocturno_fuente") else " (editable)")
    )
    discarded_night = meta.get("periodo_nocturno_descartado") or night_default_warning
    if discarded_night:
        st.warning(
            "⚠️ Se descartó un intervalo nocturno detectado de forma insegura "
            f"({discarded_night}). Se aplicará 23:00–07:00 para evitar invertir "
            "los períodos diurno/nocturno y alterar las estadísticas."
        )
    ...
    if generate:
        with st.spinner("Depurando lecturas, recalculando métricas y generando informe..."):
            ...
            qc_night_form = _validate_night_window(
                noc_start, noc_end, strict_cross_midnight=True
            )
            if not qc_night_form["ok"]:
                st.error(
                    "Intervalo nocturno inseguro: " + qc_night_form["reason"] +
                    " Use una ventana que cruce medianoche, por ejemplo 23:00–07:00."
                )
                return
            noc_start = qc_night_form["start"]
            noc_end = qc_night_form["end"]
